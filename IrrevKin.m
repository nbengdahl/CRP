...
% IrrevKin.m - an example included as a supplement to "Lagrangian 
% simulation of mixing and reactions in complex geochemical systems" 
% by Engdahl, Benson, Bolster in Water Resources Research
% 
%  Example run script to compute the irreversible kinetic reaction and 
%  generate Figure 1. Please read the description so you understand how to
%  properly define the initial conditions or your results may not match.
%
% Requires:
%    1. Control m-file    (that's this file)
%       - Note: The run file "IrrevKin_Auto.in" will be generated automatically 
...
% B is automatically calculated based on A

% ========================================================================
% This block auto-generates the required input file for PhreeqcRM 
% 
fid = fopen('IrrevKin_Auto.in','w');
...
[~]=calllib('libphreeqcrm','RM_RunFile',pfid,1,1,1,'IrrevKin_Auto.in');
...
figure;  
% Analytical solution
DAB=@(A0,kf,p,q,t) (1 + kf.*(p+q-1).*((q./p).^q).*(A0.^(p+q-1)).*t).^(1/(1-p-q));

t2=logspace(-2,2,100);
for jj=1 % Just plot A
plot(t,C_t(:,jj),'o','MarkerSize',3); hold on; grid on;
end

plot(t2,A0.*DAB(A0,kf,pp,qq,t2),'-r','linewidth',2); 
